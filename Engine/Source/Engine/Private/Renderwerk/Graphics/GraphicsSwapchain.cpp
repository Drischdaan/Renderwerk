...
VkSwapchainKHR FGraphicsSwapchain::RecreateSwapchain(const VkSwapchainKHR OldSwapchain) const
{
	...
	if (OldSwapchain != VK_NULL_HANDLE)
		DestroySwapchain();
	RW_LOG(LogGraphics, Trace, "Created swapchain with {} buffers (Format: {}, PresentMode: {})", Description.BufferCount, GetEnumValueName(Description.Format),
	       GetEnumValueName(Description.PresentMode));
	return NewSwapchain;
}
